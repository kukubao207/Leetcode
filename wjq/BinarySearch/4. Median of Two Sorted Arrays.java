...
若 A[k/2-1] > B[k/2-1]
这说明  B[k/2-1]一定不是第k大的，这很容易理解

首先B数组中比B[k/2-1]小的有 k/2 - 1 -1（去掉B[k/2-1]） +1（下标从0开始）  = k/2-1个
其次A数组中比B[k/2-1]小的最多有 k/2 - 1 -1 (去掉A[k/2-1]) + 1 （下标从0开始） = k/2-1个
那么可以推出比B[k/2-1]小的元素最多有  k/2-1 + k/2-1 = k -2 个，所以B[k/2-1]最多为第k-1大的元素

因此，我们可以把 B数组中 [0,k/2-1]的元素全部删除掉。继续递归搜索

递归的结束条件：
1.数组1为空
2.k==1，只要返回 num1[s1]和num2[s2中较小的那个就可以了
