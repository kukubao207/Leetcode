...
public class TuoPu {
    ...
    public static void main(String[] args) {
        ...
        Map<Integer, LinkedList<Integer>> dependence = new HashMap<>();
        for (int i = 1; i <= n; i++) {
            p[i] = sc.nextInt();
            dependence.put(i, new LinkedList());
        }
        for (int i = 0; i < m; i++) {
            int a = sc.nextInt();
            int b = sc.nextInt();
            dependence.get(a).add(b);//依赖关系  可能一对多
            inDegree[b]++;
        }
        ...
        while(!queue.isEmpty()){
            ...
            for(Integer d: dependence.get(pair.getValue())){
